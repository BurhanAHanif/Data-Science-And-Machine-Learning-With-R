...
mpg$trans = ifelse(str_detect(mpg$trans , "^auto"), 'automatic', 'manual')
mpg$trans
```
...
ggplot(mpg , aes(cty , trans)) +
  geom_density(aes(cty) , fill = "blue" , col = "red" , alpha = 0.5) +
                  labs(title = "City fuel efficeny for the number of cyclinders" , x = "TYPE of transmission", 
...
ggplot(mpg , aes(x = trans , y = cty)) + 
  geom_boxplot(col = "blue" , fill = "yellow" , alpha = 0.5)
```
...
ggplot(mpg , aes(x = trans , y = "" , fill = class)) + 
  ...
  labs(title = "the pie chart of transmission by Class" , subtitle = "" , x = " " , y = " " , caption = "Source EPA 2008 Fuel Economy Dataset")
```
...
a = ggplot(mpg , aes(year , hwy)) + 
  geom_count()
ggMarginal(a , type = "density" , fill = "transparent")
...
ggplot(df , aes(x , y)) +
  geom_point() + stat_function(fun  = function(x){y = x^2} , col = "blue")
```

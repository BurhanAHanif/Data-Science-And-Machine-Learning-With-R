...
ggplot(mpg,aes(cty,trans))+
  geom_density(aes(cty), fill = "blue", col = "yellow", alpha = 0.5) +
...
ggplot(df, aes(x, y)) +
  geom_point() + 
  stat_function(fun = function(x){y=x^2}, col= "blue") +
  labs(title = "Fit with Quadratic  " )
```
...
# r takes 0.5 secs and cpp takes 1.39 seconds 
```
...
# R takes 2.25 and Rcpp takes .01 secs so 300x faster
```